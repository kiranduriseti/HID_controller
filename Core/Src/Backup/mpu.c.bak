...
uint8_t mpu_read(uint8_t reg){
	data = 0;
	HAL_I2C_Mem_Read(&hi2c2, MPU_add, reg, 1, &data, 1, HAL_MAX_DELAY);
	return data;
}

void mpu_write(uint8_t reg, uint8_t d){
	HAL_I2C_Mem_Write(&hi2c2, MPU_add, reg, 1, &d, 1, HAL_MAX_DELAY);
}
...
void mpu_read_gyro(int16_t *x, int16_t *y, int16_t *z) {
	...
	HAL_I2C_Mem_Read(&hi2c2, MPU_add, multi_gyro, 1, rx, 6, HAL_MAX_DELAY);
...
void mpu_read_acc(int16_t *x, int16_t *y, int16_t *z) {
	...
	HAL_I2C_Mem_Read(&hi2c2, MPU_add, multi_acc, 1, rx, 6, HAL_MAX_DELAY);
